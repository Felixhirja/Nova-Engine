...
void CameraFollowController::ApplyFreeCameraMovement(Camera& camera,
    ...
    if (config.pitchAffectsForward) {
        const double cy = std::cos(yaw),  sy = std::sin(yaw);
        const double cp = std::cos(pitch), sp = std::sin(pitch);
        // Forward at yaw=0,pitch=0 is (0,0,-1)
        fwdX = -sy * cp;
        fwdY =  sp;
        fwdZ = -cy * cp;
    } else {
    ...
    const double upX = 0.0, upY = 1.0, upZ = 0.0;
    double rightX =  fwdZ * upY - fwdY * upZ; // = fwdZ
    double rightY =  fwdX * upZ - fwdZ * upX; // = 0
    double rightZ =  fwdY * upX - fwdX * upY; // = -fwdX
    {
        const double rlen = std::sqrt(std::max(1e-12, rightX*rightX + rightY*rightY + rightZ*rightZ));
