...
2. **Render the 3D scene** – `viewport->Render(...)` activates the current view configuration (via `ActivateView`/`ApplyViewportView`), binds the appropriate context, and draws camera debug overlays when a GL backend is available.【F:engine/MainLoop.cpp†L787-L821】【F:engine/Viewport3D.cpp†L1703-L1748】【F:engine/Viewport3D.cpp†L2283-L2315】
